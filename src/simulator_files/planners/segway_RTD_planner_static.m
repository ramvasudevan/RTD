classdef segway_RTD_planner_static < generic_RTD_planner
    properties
        point_spacing
        arc_point_spacing
        w_polynomial_info
    ...
    methods
        ...
        function load_FRS_files(P,~,~)
            FRS_data = cell(1,1) ;
            FRS_data{1} = load('segway_FRS.mat') ;
            P.FRS = FRS_data ;
        end
        ...
        function setup(P,agent_info,world_info)
            ...
            % set point spacings
            [r,a] = compute_point_spacings(bbar,P.buffer) ;
            P.point_spacing = r ;
            P.arc_point_spacing = a ;
            
            % set FRS buffer; the FRSes for the segway were solved with a
            % footprint of 0.2 m, but the actual segway has a footprint of
            % 0.38 m, so we need a different buffer distance
            P.FRS_buffer = P.buffer + (agent_info.footprint - P.FRS{1}.rcar) ;
            ...
            % process the w polynomial from the segway's FRS; this just
            % extracts the msspoly's powers and coefficients to speed up
            % evaluating the polynomial online
            w = P.FRS{P.current_FRS_index}.out.w - 1.0001 ; % this is so (x,k) \in FRS => w(x,k) > 0
        ...
        function process_world_info(P,world_info,~)
            ...
            if ~isempty(O)
                ...
                O_FRS = crop_points_outside_region(0,0,O_FRS,1,2) ;
            else
                O_FRS = [] ;
            end
        ...
        function create_cost_function(P,agent_info,world_info,start_tic)
            ...
            P.trajopt_problem.cost_function = @(k) P.cost_segway(k,w0,v0,wp_local,start_tic) ;
        end
